  ...
  <div class="challenge-container">
    <Main>
      ...
      <ModalCreateChallenge
        ...
        :onOk="() => createThuThach(modalState.currentModalID)"
        :onCancel="handleCancel"
        :modalID="modalState.currentModalID"
        ...
        <a-row :gutter="[24, 8]">
            ...
            <div class="challenge-inprogress">
              ...
              <TabBasic v-model:activeKey="activeKey">
                <Child v-for="(tab, index) in tabs" :key="index + 1">
                  <template #tab
                    ><span class="tab-label">{{ tab.label }}</span></template
                  >
                  <div class="select-status">
                    <a-select
                      v-if="index == 0"
                      v-model:value="selectState"
                      style="width: 100%"
                  ...
                  <div v-if="activeKey === tab.id" class="tab-content">
                    <a-row :gutter="[16, 16]" v-if="tab.id === 1 || tab.id === 2" class="tab-list">
                      <a-col
                        ...
                        :sm="24"
                        :xs="24"
                        v-for="(item, index) in tab.id === 1 && selectState === 'happening'
                          ? displayHappeningTour
                          : tab.id === 1 && selectState === 'upcoming'
                          ? displayUpcomingTour
                          : tab.id === 1 && selectState === 'ended'
                          ? displayEndedTour
                          : tab.id === 2
                          ? displayedSoloItems
                          : []"
                  ...
                  <div class="pagination-wrapper">
                    <sdCards v-show="tab.id == 1 && selectState == 'happening'" class="challenge-pagination">
                      <a-pagination
                        v-model="currentPageState.tournament.happening"
                        :total="happeningTourItems"
                    ...
                    <sdCards v-show="tab.id == 1 && selectState == 'upcoming'" class="challenge-pagination">
                      <a-pagination
                        v-model="currentPageState.tournament.upcoming"
                        :total="upcomingTourItems"
                        :showSizeChanger="false"
                        :pageSize="tournamentPageSize"
                        show-less-items
                    ...
                    <sdCards v-show="tab.id == 1 && selectState == 'ended'" class="challenge-pagination">
                      <a-pagination
                        v-model="currentPageState.tournament.ended"
                        :total="endedTourItems"
                        :showSizeChanger="false"
                        :pageSize="tournamentPageSize"
                        show-less-items
                    ...
                    <sdCards v-show="tab.id == 2" class="challenge-pagination">
                      <a-pagination
                        v-model="currentPageState.solo.current"
                        :total="soloTotalItems"
                        :showSizeChanger="false"
                        :pageSize="soloPageSize"
                        show-less-items
          ...
          <a-col :xxl="6" :xl="6" :lg="24" :md="24" :sm="24" :xs="24">
            <div class="ranking-top">
                ...
                <template #renderItem="{ item }">
                  <div class="ranking-user" :style="{ 'border-color': getRankingStyle(item.order).color }">
                    ...
                    <div class="ranking-user-meta">
                      <h3 class="ranking-user-title" :style="{ color: getRankingStyle(item.order).color }">
                        <img
                          v-if="getRankingStyle(item.order).icon"
                          :src="getRankingStyle(item.order).icon"
                          alt="Ranking Icon"
                          class="ranking-medal"
                        />
                      ...
                      <p class="ranking-user-description">
                        {{
                          `Elo: ${item.elo} | Attended:
                                                ${item.attended}`
                        }}
                      </p>
                    </div>
              ...
              <Buttons class="btn-ranking-view" type="primary" @click="toggleViewMore">{{
...
const challengeInProgress = {
  tournament: [
    ...
    },
    {
      ...
      poster:
        'https://yt3.googleusercontent.com/A0i8xdbzNoYrO6dGSoS3eBPUAnhKmU9MDXtS0xI2kD6buLEi8klyPfLpIkmUkhOiZ4mEAj6wSQ=s900-c-k-c0x00ffffff-no-rj',
      startTime: '2023-11-09 10:00 AM',
      status: 'happening',
    },
    {
